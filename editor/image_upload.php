...
$file_name = $_FILES['upload_image_file']['name'];                // 업로드한 파일명
$file_tmp_name = $_FILES['upload_image_file']['tmp_name'];   // 임시 디렉토리에 저장된 파일명
$file_size = $_FILES['upload_image_file']['size'];                 // 업로드한 파일의 크기
$mimeType = $_FILES['upload_image_file']['type'];                 // 업로드한 파일의 MIME Type
...
// 업로드 파일 확장자 검사 (필요시 추가)
   ...
   // 파일명 변경 (실제 업로드되는 파일명을 별도로 생성하고 원래 파일명을 별도로 변수에 지정하여 DB에 기록할 수 있습니다.)
	...
	$file_time = time(); 
	$file_Name = "image_".$file_time.".".$file_exe;	 // 실제 업로드 될 파일명 생성	(본인이 원하는 스타일로 파일명 지정 가능)	 
...
// <![CDATA[
	...
	function initUploader(){
        ...
        var _mockdata = {
            'imageurl': '<?php echo $dest_url; ?>',
            'filename': '<?php echo $change_file_name; ?>',
            'filesize': <?php echo $file_size; ?>,
            'imagealign': 'C',
            'originalurl': '<?php echo $dest_url; ?>',
            'thumburl': '<?php echo $dest_url; ?>'
        };
